...
use bitcoin::{
    ...
    hashes::Hash,
    BlockHeader, PubkeyHash, PublicKey, Script, ScriptHash, VarInt,
};
use db_key::Key;
use leveldb::{
    database::Database,
...
#[derive(StructOpt)]
#[structopt(name = "bitcoin-mdbx", about = "Convert bitcoin's database to MDBX")]
struct Opt {
    #[structopt(long, env)]
...
impl From<CompressedScript> for Script {
    fn from(script: CompressedScript) -> Self {
        match script {
            CompressedScript::P2PK(key) => Script::new_p2pk(&key),
            CompressedScript::P2PKH(keyhash) => Script::new_p2pkh(&keyhash),
            CompressedScript::P2SH(scripthash) => Script::new_p2sh(&scripthash),
...
fn main() -> anyhow::Result<()> {
    ...
    let out_db = mdbx::Environment::new()
        .set_max_dbs(100)
    ...
    let state_table = tx.create_db(Some("State"), DatabaseFlags::default())?;

    let headers_table = tx.create_db(Some("Headers"), DatabaseFlags::default())?;
    let block_hashes_table = tx.create_db(Some("BlockHashes"), DatabaseFlags::default())?;
    let bodies_table = tx.create_db(Some("Bodies"), DatabaseFlags::default())?;
    let txs_table = tx.create_db(Some("BlockTransaction"), DatabaseFlags::default())?;
    let txlookup_table = tx.create_db(Some("TxLookup"), DatabaseFlags::default())?;

    let rev_state_table = tx.create_db(Some("RevState"), DatabaseFlags::default())?;
            ...
            .skip(opt.skip.unwrap_or(0))
        {
            ...
            match letter {
                b'b' => {
                    ...
                    let header = BlockHeader::consensus_decode(&mut vcur)?;
                    ...
                    if let Some(fno) = f {
                        ...
                        if let Some(data_pos) = data_pos {
                            ...
                            let h = BlockHeader::consensus_decode(&mut f)?;
                            assert_eq!(header, h);
                            let blocktxs = Vec::<bitcoin::Transaction>::consensus_decode(&mut f)?;

                            block_body.tx_index = tx.db_stat(&txs_table)?.entries() as u64;
                            ...
                            if let Some(undo_pos) = undo_pos {
                                ...
                                block_body.rev_index =
                                    tx.db_stat(&rev_state_table)?.entries() as u64;
